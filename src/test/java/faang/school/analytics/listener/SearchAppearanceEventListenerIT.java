...
public class SearchAppearanceEventListenerIT {
    ...
    @Test
    public void testSearchAppearanceEventListener() {
        SearchAppearanceEvent event = new SearchAppearanceEvent(122L, 2L, LocalDateTime.now());
        redisTemplate.convertAndSend("search_appearance_channel", event);

        List<AnalyticsEvent> events = IterableUtils.toList(analyticsEventRepository.findAll());

        assertEquals(1, events.size());
        assertEquals(122, events.get(0).getAuthorId());
    }
