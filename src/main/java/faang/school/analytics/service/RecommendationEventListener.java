...
public class RecommendationEventListener implements MessageListener {
    ...
    public void onMessage(@NonNull Message message, byte[] pattern) {
        ...
        log.info("Message received: {}, Pattern {}", message, new String(pattern, StandardCharsets.UTF_8));
        try {
            RecommendationEvent recommendationEvent = objectMapper.readValue(message.getBody(), RecommendationEvent.class);
            AnalyticsEventDto dto = analyticsEventService.saveEvent(
                    analyticsEventMapper.fromRecommendationEventToDto(recommendationEvent)
            );
